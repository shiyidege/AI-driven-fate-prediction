...
#from rag.build_index import build_or_update_chroma
    ...
    def send_message(self, user_message: str, keep_context: bool = True) -> str:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=self.conversation_history, # 报错不影响运行，类型问题
                timeout=100.0,
                temperature=0.7  # 新增可选参数，提升兼容性
            )
            # 提取响应内容（openai 2.x 正确取值）
            content = response.choices[0].message.content
            assistant_content = content.strip() if content is not None else ""
...
with st.sidebar:
    ...
    tradition = st.selectbox(
        "体系范围",
        ["全部", "八字", "紫微斗数", "六爻", "占星","塔罗","梅花易数"],
        index=0
    )

    # （可选）一键重建索引按钮：只在更新knowledge_suanming后需要
    rebuild_index = st.button("📚 重建/更新知识库索引", type="secondary")

    persist_dir = st.text_input(
        "Chroma persist_dir",
        value="knowledge_suanming/chroma_store",
        help="向量库持久化目录（build_index.py生成）"
    )
...
if st.session_state.ai_client and (user_input := st.chat_input("请输入你的问题...")):
    ...
    with st.chat_message("assistant"):
        with st.spinner("思考中..."):
            ...
            # 体系映射成metadata里的 tradition_filter
            tradition_filter = None
            if tradition == "八字":
                tradition_filter = "bazi"
            elif tradition == "紫微斗数":
                tradition_filter = "ziwei"
            elif tradition == "六爻":
                tradition_filter = "liuyao"
            elif tradition == "占星":
                tradition_filter = "astrology"
            elif tradition == "塔罗":
                tradition_filter = "tarot"
            elif tradition == "梅花易数":
                tradition_filter = "shushu"
            else:
                tradition_filter = None  # 全部

            if use_rag:
                # 走RAG：不必把资料塞进 conversation_history（否则上下文会越来越长）
                response = rag_answer(
                    question=user_input,
                ...
                # 但为了UI展示历史，你仍然可以把这轮对话记录下来：
                st.session_state.ai_client.conversation_history.append({"role": "user", "content": user_input})
                st.session_state.ai_client.conversation_history.append({"role": "assistant", "content": response})
            else:
                # 走原聊天
                response = st.session_state.ai_client.send_message(user_input, keep_context=True)
...
# streamlit run conversation_page.py